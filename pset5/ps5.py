...
class PhraseTrigger(Trigger):
    ...
    def is_phrase_in(self, phrase, story):
        ...
        story = story.translate(translator).lower().split()
        storyphrase = []

        for item in range(len(story)):
            for word in range(len(phrase)):
                if story[item] == phrase[word]:
                        storyphrase.append(story[item])

        storyphrase = " ".join(storyphrase)
        phrase = " ".join(phrase)

        if storyphrase == phrase:
            return True
        else:
            return False
...
class TitleTrigger(PhraseTrigger):
    ...
    def __init__(self):
        PhraseTrigger.__init__(self, self.phrase)
